...
short LED_centre_steering = 3; // TBD

short pin_throttle = 9;
short LED_neutral = 6; // TBD
short LED_forwards = 7; //TBD
short LED_brakes = 5;
...
void turn_LED_switch(int val1, int val2, int val3) {
    ...
    digitalWrite(LED_centre_steering, val1); //TBD
    digitalWrite(LED_left_steering, val2);
    digitalWrite(LED_right_steering, val3);
}

void throttle_LED_switch(int val1, int val2, int val3) { //TBD
    digitalWrite(LED_neutral, val1); //TBD
    digitalWrite(LED_forwards, val2); //TBD
    digitalWrite(LED_reverse, val3);
}
...
void setup() {
    ...
    pinMode(LED_centre_steering, OUTPUT); //TBD

    pinMode(LED_neutral, OUTPUT); //TBD
    pinMode(LED_forwards, OUTPUT); //TBD
...
void loop() {
    ...
    ///////////////////////////////////////////////////
     if (c_throt_pos == neutral) { //TBD
        throttle_LED_switch(HIGH, LOW, LOW);}
    else if (c_throt_pos == forwards) { //TBD
        throttle_LED_switch(LOW, HIGH, LOW);}
    else if (c_throt_pos == reverse) {
        throttle_LED_switch(LOW, LOW, HIGH);}
    // (c_throt_pos == reverse) ? throttle_LED_switch(LOW, LOW, HIGH) : throttle_LED_switch(LOW, LOW, LOW);
